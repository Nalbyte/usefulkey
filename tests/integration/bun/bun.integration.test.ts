...
async function waitForServer(url: string, maxAttempts = 40, delayMs = 250): Promise<void> {
  ...
  throw new Error(
    `Bun example server not reachable at ${url}. Ensure it is running (docker compose or GH Actions). Last error: ${String(
      lastErr,
    )}`,
  );
}
...
describe("Bun example endpoints", () => {
  it("/health responds with ok:true and service name", async () => {
  ...
  it("POST /admin/keys returns an id and key", async () => {
    const res = await fetch(`${baseUrl}/admin/keys`, {
      method: "POST",
      headers: { 
        "content-type": "application/json",
  ...
  it("POST /admin/keys/pro creates a pro key", async () => {
    const res = await fetch(`${baseUrl}/admin/keys/pro`, {
      method: "POST",
      headers: { 
        "content-type": "application/json",
        "x-admin-key": ADMIN_KEY
  ...
  it("POST /admin/keys/premium creates a premium key with premium scope", async () => {
    const res = await fetch(`${baseUrl}/admin/keys/premium`, {
      method: "POST",
      headers: { 
        "content-type": "application/json",
        "x-admin-key": ADMIN_KEY
  ...
  it("POST /admin/keys/admin creates an admin key with admin and premium scopes", async () => {
    const res = await fetch(`${baseUrl}/admin/keys/admin`, {
      method: "POST",
      headers: { 
        "content-type": "application/json",
        "x-admin-key": ADMIN_KEY
  ...
  it("GET /admin/keys/:id returns key info", async () => {
    ...
    const res = await fetch(`${baseUrl}/admin/keys/${keyId}`, {
  ...
  it("PUT /admin/keys/:id/enable enables a key", async () => {
    const keyId = createdKey?.id ?? "";
    expect(keyId).toBeTruthy();
    const res = await fetch(`${baseUrl}/admin/keys/${keyId}/enable`, {
      method: "PUT",
      headers: { "x-admin-key": ADMIN_KEY },
    });
    expect(res.ok).toBe(true);
  ...
  it("PUT /admin/keys/:id/disable disables a key", async () => {
    const keyId = createdKey?.id ?? "";
    expect(keyId).toBeTruthy();
    const res = await fetch(`${baseUrl}/admin/keys/${keyId}/disable`, {
      method: "PUT",
      headers: { "x-admin-key": ADMIN_KEY },
    });
    expect(res.ok).toBe(true);
  ...
  it("GET /admin/keys/:id/scopes returns scopes", async () => {
    const keyId = premiumKey?.id ?? "";
    expect(keyId).toBeTruthy();
    const res = await fetch(`${baseUrl}/admin/keys/${keyId}/scopes`, {
      headers: { "x-admin-key": ADMIN_KEY },
    });
    expect(res.ok).toBe(true);
